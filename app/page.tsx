      ...
      <section className="bg-paper pb-20 pt-4">
        <div className="container-page">
          ...
          <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {pillars.map((p, i) => {
              const accents = ['bg-teal-500', 'bg-sun-400', 'bg-bloom-400'];
              const accent = accents[i % accents.length];
              return (
                <div key={p.title} className="overflow-hidden rounded-xl2 bg-white shadow-card">
          ...
          <div className="mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {admissionsSteps.map((s, i) => (
              <div key={s.step} className="relative pl-0">
                <span className="flex h-9 w-9 items-center justify-center rounded-full bg-teal-500 font-mono text-xs font-semibold text-white">
                  {s.step}
                </span>
                ...
                {i < admissionsSteps.length - 1 && (
                  <span className="absolute -right-4 top-2 hidden text-ink/20 lg:block">—</span>
                )}
              </div>
            ))}
          </div>
